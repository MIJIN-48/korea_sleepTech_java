 ...
 * 		: 상품 추가, 조회, 수정, 삭제와 같은 비즈니스로 직직을 처리(직접적인 데이터 조작)
...
public class Main {
	...
	private static void addItem(InventoryService inventoryService, Scanner sc) {
		...
		if (category.equalsIgnoreCase("Electronics")) {
			...
			inventoryService.addItme(electronics);
			...
			inventoryService.addItme(furniture);
		} else {
	...
	private static void viewItemsBycategory(InventoryService inventoryService, Scanner sc) {
		...
		if (items.isEmpty()) {
			...
			System.out.println("Item in category: " + category + ">>> ");
			for (Item item : items) {

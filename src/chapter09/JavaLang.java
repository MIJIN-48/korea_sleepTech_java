...
class MyClass extends Object {
	...
	// : 두 객체가 동등한지 배교
	// - 두 객체의 참조를 비교 (동일한 주소 값을 가지는지 확인)
}
...
public class JavaLang {
	public static void main(String[] args) {
		...
		MyClass myClass1 = new MyClass(1, "박미진");
		System.out.println(myClass1);
		// MyClass(ID: 1 / NAME: 박미진)
		// : 객체를 출력하는 경우 자동으로 toSting() 호출
		
		System.out.println(myClass1.toString()); // MyClass(ID: 1 / NAME: 박미진)
		
		System.out.println(myClass1.displayInfo()); // ID: 1 / NAME: 박미진
		
		MyClass myClass2 = new MyClass(2, "홍길동");
		...
		// : 무작위 난수를 생성: .random() = 0.0이상 0.1 미만의 난수를 생성
		...
		boolean isEquals = message.equals("Merry Cristmas^^");
		System.out.println(isEquals); // false
		
		// substring(int start, int end)
		// : 시작 인덱스(포함)부터 끝 인덱스(미포함)까지의 부분 문자열을 반환
		// - 문자열 인덱스 번호는 0부터 시작, 공백 기호까지 포함

...
public class Practice01 {
	public static void main(String[] args) {
		List<StudentClass> studentCalsses = Arrays.asList(
		...
		List<StudentClass> filteredStudents = studentCalsses.stream()
		...
		// 2) 학생 이름 목록만 추출
		List<String> studentNames = studentCalsses.stream()
//				.map(student -> student.getName())
		...
		// >> Comparator 클래스. comparing타입 정적 메서드 활용
		List<StudentClass> sortedStudents = studentCalsses.stream()
				.sorted(Comparator.comparingDouble(StudentClass::getGrade)) // 오름차순
//				.sorted(Comparator.comparingDouble(StudentClass::getGrade).reversed()) // 역순 정렬
				.collect(Collectors.toList());
		...
		List<StudentClass> studentsWithJi = studentCalsses.stream()
				.filter(studentCalss -> studentCalss.getName().contains("지"))
				.collect(Collectors.toList());
